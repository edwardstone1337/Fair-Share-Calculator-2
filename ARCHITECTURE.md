...
- **`/`** — Root layout wraps with `CurrencyProvider` and renders sticky `NavBar` (logo + title at ≥420px; desktop ≥640px: Calculator/FAQ links absolutely centred; mobile <640px: Menu button + dropdown; right section order: CurrencySelector on `/` then Menu; auth UI currently hidden, preserved for re-enablement). Server-rendered calculator page (H1, `CalculatorClient`, JSON-LD). Client: `CalculatorClient` uses `useCalculator()` and `useCurrency()`; reducer + localStorage restore on mount; currency from context, persisted in `fairshare_currency`. URL hash (`#results`, `#input`) syncs step; URL params (`?config=`) load a saved configuration. **Save flow (results)**: Save button is only shown when `NEXT_PUBLIC_AUTH_ENABLED === 'true'` (e.g. in .env.local). When that flag is unset (production), the button is not rendered so anonymous users never hit the dead-end `/login` redirect. When enabled: anonymous users get `fairshare_pending_save` set and redirect to `/login`; logged-in users call `saveConfiguration`, then show "Saved" or error via snackbar. **Config loading**: `?config={id}` triggers `getConfiguration(id)`; on success form state is restored and step set to input for review; URL is cleaned (param removed). **No auth gate** — calculator works for everyone.
- **`/faq`** — FAQ page (server-rendered): 10 FAQs, couples-focused copy, FAQPage JSON-LD; "Try the calculator" CTAs after key FAQs (secondary button style, 48px touch target, href `/`), each with unique GA4 `source` for attribution (e.g. `faq_how_to_use`, `faq_rent`, `faq_closing`); includes an "About This Site" section with support copy, Buy Me a Coffee links, and feedback survey CTAs (`faq_cta_clicked` with `cta` values `feedback_survey` and `feedback_survey_button`); uses `--faq-*` tokens; floating `BackToTopButton` (fixed bottom-right, icon-only, appears after 400px scroll).
...
| `app/faq/page.tsx` | FAQ page: metadata, 10 FAQs, FAQPage JSON-LD; "Try the calculator" CTAs (secondary style) with optional `source` (faq_how_to_use, faq_rent, faq_closing, etc.) for faq_cta_clicked; About This Site section with Buy Me a Coffee and feedback survey CTAs (`feedback_survey`, `feedback_survey_button`); token-driven (`--faq-*`) |
...
| `lib/hooks/use-input-tracking.ts` | Per-field analytics: input_started, input_completed (no validation_error; that fires on Calculate only in calculator-client); prefilled opt-out |
| `lib/analytics/gtag.ts` | trackEvent, bucketExpenseAmount, bucketSplitRatio; GA4 wrapper |
| `components/calculator/calculator-client.tsx` | Orchestrates sections, step (input vs results), URL hash/params, snackbar; fires validation_error (submit-time only, field-level params) and calculate_attempt; scheduleScrollToError wraps scrollToFirstError and clears timeout on unmount |
...
| `components/back-to-top-button.tsx` | Reusable floating back-to-top FAB: fixed bottom-right, icon-only (aria-label), 400px scroll threshold (optional `threshold`), fade+slide animation; token-only styles; used on FAQ page |
| `components/calculator/results-footer.tsx` | Results actions/footer: support CTA card + Buy Me a Coffee image link, Edit details button, feedback survey button (`feedback_clicked`), optional Save button state |
